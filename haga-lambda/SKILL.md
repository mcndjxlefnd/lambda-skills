---
name: haga-lambda
description: Lambda skill — HAGA codebase knowledge: repo layout, constraints,
...
- **Convergence detection: integral-of-|f'|, not pairwise delta.** `avg|f'|` over sliding window
  (stdlib, K=5, ε=0.1, W=5 consecutive). Pairwise flags plateaus; slope is threshold-sensitive.
  Integral gives identical results for ε=0.05–1.0. Present method before implementing.
- **GA operator changes: analyze dynamics first.** Don't jump to "spec says X, code does Y."
  ...
  the "bug" is beneficial. Correctness ≠ fix.
